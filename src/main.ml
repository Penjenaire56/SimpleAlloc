...
type _var = string
type _val = int
type src = IO of _var * string * int | Var of _var | Val of _val
type dst = DVar of _var | DIO of _var * string * int
...
type index = src
type inst = Oper of op * dst * src * src | Move of dst * src

let srcToStr (exp : src) =
  match exp with Val i -> string_of_int i | Var s -> s | IO (s, _, _) -> s

let dstToStr (exp : dst) = match exp with DVar s -> s | DIO (s, _, _) -> s
...
let defN = function Move (dst, _) | Oper (_, dst, _, _) -> dstToStr dst

let srcToStrOpt (a : src) =
  match a with Var a | IO (a, _, _) -> Some a | Val _ -> None
...
let useN = function
  ...
  | Oper (_, _, s1, s2) -> (
      ...
      | None, Some s | Some s, None -> SSet.singleton s
      | Some s1, Some s2 -> SSet.add s1 (SSet.singleton s2))
...
let pprintInst (i : inst) =
  ...
  | Move (dst, src) -> printDst dst ^ " = " ^ printSrc src
...
module TimerInstTest = Timer.Make (Config)
...
let instTest (i1 : inst list) (i2 : inst list) =
  ...
  let rec instToTest (i : inst list) (spe : string) v_out v_in =
    ...
    let dstInEnv dst env =
      match dst with
      | DIO (s, _, _) -> (SMap.add s (spe ^ s) env, spe ^ s)
      | DVar s -> (env, spe ^ s)
    in
    ...
    | x :: subL -> (
        ...
        | Move (dst, src) ->
            ...
            append (instToTest subL spe v_out v_in) ((dst ^ " = " ^ src) ^ "\n")
...
let cleanName name = String.concat "" (String.split_on_char '\'' name)
let freshName (k : int) = (k + 1, "tmp" ^ string_of_int k)
...
let dstName (name : string) (v_io : SSet.t) (io : (bool * string) SMap.t) v2 k =
  if SSet.mem name v_io then
    if SMap.mem name io then
      ...
      if b then (k, io, [ Move (DVar tmp, v2) ])
      else
        let n, x = getIOName name in
        ( k,
          SMap.add name (true, tmp) io,
          [ Move (DVar tmp, v2); Move (DIO (name, n, x), Var tmp) ] )
    else
      let k, tmp = freshName k in
      let n, x = getIOName name in
      ( k,
        SMap.add name (true, tmp) io,
        [ Move (DVar tmp, v2); Move (DIO (name, n, x), Var tmp) ] )
  else (k, io, [ Move (DVar name, v2) ])

let rec numToInstr (n : arith_expr) (k : int) (v_io : SSet.t)
    (io : (bool * string) SMap.t) =
  match n with
...
let rec varToInstr (v : var) (k : int) (v_io : SSet.t)
    (io : (bool * string) SMap.t) =
  match v with
  | Var id ->
  ...
  | Index (v, n) ->
      ...
      let v_io = if SSet.mem v1 v_io then SSet.add name v_io else v_io in

      (k, v_io, io, i1 @ i2, name)
  | _ -> failwith "varToINst"

let rec exprToInst (e : expr) (k : int) (v_io : SSet.t)
    (io : (bool * string) SMap.t) =
  match e with
  | Const (i, _) -> (k, v_io, io, [], Val i)
  | ExpVar v ->
...
let deq_iToInst (d : deq_i) (k : int) (v_io : SSet.t)
    (io : (bool * string) SMap.t) =
  match d with
  | Eqn (v, exp, _) -> (
      match v with
...
type stability = Stable | UnStable | Mixed of (int * int) list
type structSize = Len of int | Dim of int * structSize
...
let defToInst (d : def) (max : int -> bool) shape =
  let v_io =
    List.fold_right
      (fun x s -> SSet.add (cleanName x.vd_id.name) s)
      d.p_in SSet.empty
  in
  let v_io =
    List.fold_right
  ...
  let rec apply shape lp =
    match (shape, lp) with
    | [ _ ], [ d ] -> int_of_string d
    | s :: shape, d :: lp -> (s * int_of_string d) + apply shape lp
    | _ -> failwith "error shape"
  in

  let f x (i, o) =
    match String.split_on_char '_' x with
    | "key" :: _ :: _ -> (SMap.add x true i, o)
    | "plain" :: lp ->
        if List.length lp <> List.length shape then (SMap.add x false i, o)
        else
          let n = apply (List.rev shape) lp in
          (SMap.add x (max n) i, o)
    | "cipher" :: _ -> (i, SSet.add x o)
    | _ -> (i, o)
  in
...
let addVars (v : SSet.t) (n : string) = SSet.add n v
...
let parse (path : string) =
  ...
  let rec parse_line v = function
    ...
    | x :: subL ->
        ...
        if len < 2 then parse_line v subL
        else
          let v, i =
            if len = 5 then getOper line v
            else if len = 3 then getMove line v
            else failwith "format line"
          in
          let v, l = parse_line v subL in
          (v, i @ l)
  in

  parse_line SSet.empty lines
...
and aux_insert (instrs : inst list) (name : string) (x : inst) =
  ...
  | i :: subI -> (
      ...
      | Oper (_, _, Var s1, Var s2) when s1 = name || s2 = name ->
          x :: i :: aux_reduce subI
      | Move (_, Var s) when s = name -> x :: i :: subI
      | _ -> i :: aux_insert subI name x)

let reduce (instrs : inst list) =
  let rec aux instrs k =
    if k > 0 then aux (aux_reduce instrs) (k - 1) else instrs
  in
  aux instrs 1000
...
let getFresh (mem : memory) n =
  if IMap.is_empty mem.free then
    ...
    let k, v = IMap.choose mem.free in
    {
      k = mem.k;
      max = mem.max;
...
let rec aux_update newest (mem : memory) =
  if SSet.is_empty newest then mem
  else
...
let update def newest (mem : memory) =
  ...
  | Some def -> aux_update newest (mem_free mem def)
...
let getRegistreNameDST (var : dst) (binds : int SMap.t) =
  match var with DVar s -> DVar (getRegistreName s binds) | _ -> var

let getRegistreNameSRC (var : src) (binds : int SMap.t) =
  match var with Var s -> Var (getRegistreName s binds) | _ -> var
...
let srcInSSet (expr : src) (s : SSet.t) =
  match expr with IO (x, _, _) | Var x -> SSet.mem x s | _ -> false

let dstInSSet (expr : dst) (s : SSet.t) =
  match expr with DIO (x, _, _) | DVar x -> SSet.mem x s
...
let distribute (i : inst) (prevbinds : int SMap.t) (newest : SSet.t) mem =
  ...
  | Move (dst, src) ->
      ( [
          Move
            (getRegistreNameDST dst prevbinds, getRegistreNameSRC src mem.binds);
        ],
        mem )
  | Oper (op, d, s1, s2) when srcInSSet s1 newest && srcInSSet s2 newest ->
      let tmp1 = "tmp1" in
      let mem = aux_update (SSet.singleton tmp1) mem in
      let rdest, rserc1, rserc2 =
        getRegistreNames (d, s1, s2) prevbinds mem.binds
      in
      ...
      let stmp1 =
        getRegistreNameSRC
          (match s1 with IO (_, _, _) -> s1 | _ -> Var tmp1)
          mem.binds
      in

      ( [ Oper (op, rdest, stmp1, rserc2); Move (rtmp1, rserc1) ],
        mem_free mem tmp1 )
...
let clean prog =
  List.filter (function Move (DVar d, Var s) -> d <> s | _ -> true) prog
    ...
    usage =
      SSet.fold
        (fun s u ->
          ...
          match v with None -> u | Some v -> IMap.add k (v + 1) u)
        vars mem.usage;
    binds = mem.binds;
    regs = mem.regs;
  }
...
let analyse (instr : inst list) (initials : SSet.t) (max : int) =
    ...
    | [] ->
        ...
        let rec split k = function
          ...
          | (x, _) :: subL ->
              ...
              if k > 0 then (x :: l1, l2) else (l1, x :: l2)
        in

        let regAcc, stackAcc = split mem.max acc in
        ( ( List.map (fun n -> "r" ^ string_of_int n) regAcc,
            List.map (fun n -> "r" ^ string_of_int n) stackAcc ),
          [] )
    | i :: subL ->
        ...
        let instrs, mem =
          distribute i previousBind (SSet.diff use newest) mem
        in
  ...
  let optiProg = clean prog in
  ((regAcc, stackAcc), optiProg)

let slpToJasmin dst prog stack reg head id =
  let header =
    SSet.fold
      (fun x str -> str ^ ", reg u64 " ^ x)
      head
      ("export fn p" ^ string_of_int id ^ "(reg u64 _tmp")
    ^ "){\n"
  in
  let footer = "\n}\n" in
  let declaration =
    List.fold_right
      (fun n str -> str ^ "\tstack u64 " ^ n ^ ";\n")
      stack
      (List.fold_right (fun n str -> str ^ "\treg u64 " ^ n ^ ";\n") reg "")
  in

  let body =
    List.fold_right (fun x str -> str ^ "\t" ^ pprintInst x ^ ";\n") prog ""
  in
...
let slpToC dst prog stack reg head id =
  let header =
    SSet.fold
      (fun x str -> str ^ ", uint64_t* " ^ x)
      head
      ("void p" ^ string_of_int id ^ "(uint64_t* _tmp")
    ^ "){\n"
  in
  let footer = "\n}\n" in
  let declaration =
  ...
  let body =
    List.fold_right (fun x str -> str ^ "\t" ^ pprintInstC x ^ "\n") prog ""
  in
...
let stackAlloc (i : inst) (stack : SSet.t) =
  match i with
  | Oper (o, dst, s1, s2)
    when dstInSSet dst stack && srcInSSet s1 stack && srcInSSet s2 stack ->
      [
        Move (DVar "tmp", s1);
  ...
  | Oper (o, dst, s1, s2) when srcInSSet s1 stack && srcInSSet s2 stack ->
      [ Move (DVar "tmp", s1); Oper (o, dst, Var "tmp", s2) ]
  ...
  | Oper (o, dst, s1, s2) when dstInSSet dst stack ->
      [ Oper (o, DVar "tmp", s1, s2); Move (dst, Var "tmp") ]
  | Oper (o, dst, s1, s2) when srcInSSet s1 stack -> [ Oper (o, dst, s2, s1) ]
  | i -> [ i ]
...
let isStr (a : src) (s : string) =
  match a with Val _ -> false | Var a | IO (a, _, _) -> a = s

let dstToStr (a : dst) = match a with DVar a | DIO (a, _, _) -> a

let pickWorst (t : SSet.t) (prog : inst list) (used : SSet.t) =
  let dstisStr (d : dst) s = match d with DVar d | DIO (d, _, _) -> d = s in
  let rec pick (s : string) k = function
    | [] -> k
    | i :: subL -> (
        match i with
        | Move (dst, src) when dstisStr dst s || isStr src s -> k
        | Oper (_, dst, s1, s2) when dstisStr dst s || isStr s1 s || isStr s2 s
          ->
            k
        | _ -> pick s (k + 1) subL)
  in

  let default_value s = match SSet.mem s used with true -> -40 | false -> 0 in

  let lp =
    SSet.fold (fun k l -> (k, pick k (default_value k) prog) :: l) t []
  in

  let lp = List.sort (fun (_, x) (_, y) -> Int.compare y x) lp in
  let k, v = List.hd lp in

  (* TODO BETTER SPILLING *)
  if v > 5 then Some k else None

let rename (name : string) (reg : string) (sta : string) =
  if name = reg then sta else if name = sta then reg else name
...
let spill (i : inst) (sta : string) (reg : string) =
  let rename_src src =
    match src with Var src -> Var (rename src reg sta) | _ -> src
  in

  let rename_dst dst =
    match dst with DVar dst -> DVar (rename dst reg sta) | _ -> dst
  in

  match i with
  | Move (dst, src) -> Move (rename_dst dst, rename_src src)
  | Oper (o, dst, s1, s2) ->
...
let spilling (prog : inst list) (accReg : SSet.t) (accStack : SSet.t) =
  let update_used (used : SSet.t) reg stack =
    ...
    | None, None -> SSet.add reg used
    | Some _, None -> SSet.add stack used
    | None, Some s -> SSet.add reg (SSet.remove s used)
    | _ -> used
  in

  let rec aux (prog : inst list) (accReg : SSet.t) (accStack : SSet.t)
      (used : SSet.t) =
    match prog with
    | [] -> []
    | i :: subL -> (
        ...
        | stack when SSet.mem stack accStack -> (
            ...
            | None -> stackAlloc i accStack @ aux subL accReg accStack nused
            | Some reg ->
                ...
                if SSet.mem stack (useN i) then
                  ...
                  @ aux subL accReg accStack nused)
        | reg ->
            stackAlloc i accStack @ aux subL accReg accStack (SSet.add reg used)
        )
  in
  aux prog accReg accStack SSet.empty

let generate (instrs : inst list) (init : bool SMap.t) =
  let testSrc (exp : src) env =
    match exp with
    | Val _ -> true
    | Var s -> SMap.find s env
    | IO (x, _, _) -> SMap.find x env
  in
  ...
  let rec aux instrs env _f f_ io k =
    ...
    | i :: subL -> (
        ...
        | Move (dst, src) ->
            ...
            else aux subL (addDst dst false env) _f (i :: f_) io k
        | Oper (o, dst, s1, s2) -> (
            match (testSrc s1 env, testSrc s2 env) with
            | true, true -> aux subL (addDst dst true env) (i :: _f) f_ io k
            | false, true ->
                let tmp = "_tmp" ^ string_of_int k in
                let _f = Move (DIO (tmp, "_tmp", k), s2) :: _f in
                let f_ =
                  Oper (o, dst, s1, Var "tmp_")
                  :: Move (DVar "tmp_", IO (tmp, "_tmp", k))
                  :: f_
                in
                let k = k + 1 in
            ...
            | true, false ->
                ...
                let f_ =
                  Oper (o, dst, Var "tmp_", s2)
                  :: Move (DVar "tmp_", IO (tmp, "_tmp", k))
                  :: f_
                in
                let k = k + 1 in
                aux subL (addDst dst false env) _f f_ (SSet.add tmp io) k
            | false, false -> aux subL (addDst dst false env) _f (i :: f_) io k)
        )
  in
...
let propagate (instrs : inst list) (init : bool SMap.t) =
  let testSrc (exp : src) env =
    match exp with Val _ -> true | IO (s, _, _) | Var s -> SMap.find s env
  in

  let addDst (exp : dst) b env = SMap.add (dstToStr exp) b env in
  ...
  let aux_propagate (i : inst) (env : bool SMap.t) (out : bool SMap.t) =
    ...
    | Move (dst, src) -> (
        ...
        ( addDst dst b env,
          match dst with DIO (s, _, _) -> SMap.add s b out | _ -> out ))
    | Oper (_, dst, s1, s2) ->
...
let rec clearAcc acc instrs =
  ...
  | [] -> []
  | x :: subL ->
      if List.for_all (fun i -> not (SSet.mem x (useN i) || x = defN i)) instrs
      then clearAcc subL instrs
      else x :: clearAcc subL instrs
...
let buildUA instrs (dst : string) outs head id =
  ...
  Printf.printf "save...\n";
  slpToC dst (List.rev progFinal)
    (clearAcc accStack progFinal)
    ("tmp" :: accReg) head id;
  (test1, test2)
...
let rec findFirst max (k : int) (f : int -> int option) =
  if k < max then
    match f k with Some s -> Some s | None -> findFirst max (k + 1) f
  else None
...
let rec firstInter list name env y start =
  match list with
  | [] -> if SMap.find name env || y < start then None else Some y
  | x :: subL ->
...
let countLogic (instrs : inst list) =
  let rec aux k = function
    | [] -> k
    | Oper (_, _, _, _) :: subL -> aux (k + 1) subL
    | _ :: subL -> aux k subL
  in
  aux 0 instrs
...
let renameio ma name old ins =
  SMap.fold
    (fun k v m ->
      let n =
        match String.split_on_char '_' k with
...
let interference (instrs : inst list) result shape start sizeMax p =
  let addToEnv env out src b =
    ...
    | IO (s, n, _) ->
        (SMap.add s b env, if n = "plain" then SMap.add s b out else out)
  in
...
let getCalcPass k biti bit p2 p1 =
  let f_ =
    Float.mul (Float.pow 2. (Float.of_int (biti - k))) (Float.of_int p2)
  in
  let _f =
    Float.mul (Float.pow 2. (Float.of_int (k + biti - bit))) (Float.of_int p1)
  in
  Float.add f_ _f
...
let getDatas (src : string) shape max limit sizeMax nbpass =
  ...
  let aux_get sexpr k =
    ...
    let reference =
      Float.mul (Float.pow 2. (Float.of_int biti)) (Float.of_int porte)
    in

    let rec pass ins result p res test =
      if
        SMap.exists (fun _ v -> v) result
        && SMap.exists (fun _ v -> not v) result
      then (
        ...
        pass ins result (p + 1) (Float.add res opti)
          (instTest instrs (f_ @ _f) :: test))
      else
        ( Float.add res (Float.mul reference (Float.of_int (nbpass - p - 1))),
          test )
    in
    let opti, tests =
      pass ins result 0 (getCalcPass k biti biti porte1 porte2) test
    in
    ...
    Printf.printf "  %F : %F = %F \n" opti nopti
      (Float.mul 100. (Float.div (Float.sub opti nopti) nopti))
  in
...
let buildUA0 (src : string) (dst : string) =
  ...
  Printf.printf "parsing...\n";
  let (head, ins, outs), instrs =
    sexprToInst sexpr (fun x -> x >= 32) [ 128 ]
  in
...
module TimerMain = Timer.Make (Config)
...
let mainGetDatas () =
  ...
  (* Format.eprintf "rectangle @.";
   * getDatas "test/rectangle_short.ua0" [ 4; 16 ]
   *   (fun x k -> x <= 63 - k)
   *   17 64 10;
   *
   * Format.eprintf "present @.";
   * getDatas "test/present_short.ua0" [ 64 ] (fun x k -> x <= 63 - k) 64 64 10;
   *
   * Format.eprintf "xoodoo @.";
   * getDatas "test/xoodoo_short.ua0" [ 3; 4; 16 ]
   *   (fun x k -> x <= 75 - k)
   *   24 192 10;
   *
   * Format.eprintf "gift_bitslice @.";
   * getDatas "test/gift_bitslice_short.ua0" [ 4; 32 ]
   *   (fun x k -> x <= 127 - k)
   *   32 128 10;
   *
   * Format.eprintf "photon_bitslice @.";
   * getDatas "test/photon_bitslice_short.ua0" [ 8; 8; 4 ]
   *   (fun x k -> x <= 87 - k)
   *   24 256 10;
   *
   * Format.eprintf "ace_bitslice_short @.";
   * getDatas "test/ace_bitslice_short.ua0" [ 5; 2; 32 ]
   *   (fun x k -> x <= 161 - k)
   *   132 320 10; *)
  TimerMain.pause ()
...
let main () =
  ...
  if Config.profiling then (
    Format.printf "@.---------------@.Times: @.";
    Format.printf "├─ Time for mainGetDatas: %a@." Timer.print_time
      (TimerMain.get ());
    Format.printf "├─ Time for instTest: %a@." Timer.print_time
      (TimerInstTest.get ());
    Format.printf "└─ Time for buildTest: %a@." Timer.print_time
      (TimerBuildTest.get ()))
